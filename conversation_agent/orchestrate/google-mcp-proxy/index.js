...
// Accept key from env var OR as the first CLI argument (for npx invocation where
// env vars cannot be set: `npx -y @dipeshc/google-mcp-proxy <API_KEY>`)
const API_KEY = process.env.GOOGLE_API_KEY || process.argv[2];

if (!API_KEY) {
  process.stderr.write(
    "ERROR: Google API key not provided.\n" +
    "Pass it as an argument:  npx -y @dipeshc/google-mcp-proxy <YOUR_KEY>\n" +
    "Or set env var:          GOOGLE_API_KEY=<YOUR_KEY> node index.js\n"
  );
  process.exit(1);
}
